...
BraTS2021 MRI 데이터를 활용한 2D U-Net 기반 뇌종양 영역 분할(Segmentation) 프로젝트입니다.
FLAIR, T1, T1CE, T2 네 가지 MRI modality를 입력받아 종양 영역 mask를 예측합니다.

> **⚠️ 코드 복원 안내**
> 본 프로젝트는 바이오헬스 프런티어 프로젝트(2026) 1주차에 진행했으나,
> 프로젝트 종료 후 원본 코드를 삭제하여, 당시 작업 기록과 결과물을 토대로 파이프라인을 재구성한 버전입니다.
> 아래 성능 수치(Dice 0.855 / IoU 0.762)는 **원 프로젝트 당시 실제 학습으로 얻은 결과**이며,
> 본 재구성 코드의 재현 결과와는 차이가 있을 수 있습니다.
